...
<div id="insert" title="글 작성">
	...
	<form id="image-post" name="mForm" method="post" enctype="multipart/form-data">
		...
		<input type="text" id="title1" name="title" placeholder="제목" ><br>
		<textarea name="contents" id="contents1" class="notice_ct" placeholder="내용" ></textarea>
		<input type="button" class="button" value="파일추가" onClick="fn_addFile()"/><br>
		<div id="d_file"></div>
	</form>
	</div>
</div>
...
<div id="detail" title="글 상세보기" >
	<div class="detail">
  		...
  		<li id="commentItem"  style="display: none; list-style-type: none;" >
			<div>
				<input type="hidden" id="commentid5" value="${comment.commentid}" >
			</div>
			<div>
...
<div id="update" title="글 수정">
    ...
    <textarea name="contents" id="contents3" placeholder="내용" class="notice_ct">${board.contents}</textarea>
    <!-- ${loginMember.id}-->
...
<div id="reply" title="답글 작성">
	<div class="write">
		...
		<input type="text" id="title4" name="title2" placeholder="제목" class="text_title"><br>
		<textarea name="contents2" id="contents4" placeholder="내용" class="notice_ct"></textarea>
	</div>
</div>
...
document.querySelector("#moreBtn").addEventListener("click", () => {
    // 댓글 더보기 버튼이 클릭될 때 작동하는 부분

    // 마지막 댓글 아이템을 선택
    const lastComment = document.querySelector("#commentListUl li:last-child");
    const lastCommentId = lastComment ? lastComment.querySelector("#commentid5").innerText : null;

    // 서버에 요청을 보낼 파라미터 설정
    const param = {
        boardid: lastCommentId
    }
    ...
    .then((json) => {
        if (json.status) {
            // 서버에서 받아온 댓글 목록
            const commentList = json.comment;
            const commentListHTML = document.querySelector("#commentListUl");

            // 댓글 목록을 돌면서 각 댓글을 추가
            commentList.forEach((comment) => {
                // 댓글 아이템 템플릿을 복제
                const newCommentItem = document.querySelector("#commentItem").cloneNode(true);
                newCommentItem.style.display = "";

                // 댓글 내용, 작성자, 작성날짜, 댓글번호를 설정
                ...
                newCommentItem.querySelector("#commentid5").innerText = comment.commentid;

                // 댓글 목록에 새로운 댓글 아이템 추가
